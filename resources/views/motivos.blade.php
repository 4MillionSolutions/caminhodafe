...
@if(isset($tela) and $tela == 'pesquisa')
    <div class="right_col" role="main">
        <form id="filtro" action="motivos" method="get" data-parsley-validate="" class="form-horizontal form-label-left" novalidate="">
            <div class="form-group row">
                ...
                <table class="table table-striped text-center">
                  ...
                  @if(isset($motivos))
                        @foreach ($motivos as $motivo)
                            <tr>
                            <th scope="row" title="Editar motivo"><a href={{ URL::route($rotaAlterar, array('id' => $motivo->id )) }}>{{$motivo->id}}</a></th>
                              <td>{{$motivo->nome}}</td>
                              <td>{{$motivo->ativo == 'A' ? 'Ativo' : 'Inativo'}}</td>
...
@else
    ...
    <form id="{{ $tela }}" action="{{ $tela == 'alterar' ? $rotaAlterar : $rotaIncluir }}" method="post">
        @csrf
        @if($tela == 'alterar')
            <input type="hidden" name="id" value="{{ $motivos[0]->id ?? '' }}">
        @endif
        <div class="container">
            <div class="row row-cols-md-3 g-3">
                <div class="col-md-4">
                    <label for="pessoa" class="form-label">Nome*</label>
                    <input type="text" class="form-control" id="nome" name="nome" maxlength="200" required value="{{ $motivos[0]->nome ?? '' }}">
                </div>
            </div>
