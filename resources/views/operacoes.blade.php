...
@if(isset($tela) and $tela == 'pesquisa')
    <div class="right_col" role="main">
        <form id="filtro" action="operacoes" method="get" data-parsley-validate="" class="form-horizontal form-label-left" novalidate="">
            <div class="form-group row">
                ...
                <table class="table table-striped text-center">
                  ...
                  @if(isset($operacoes))
                        @foreach ($operacoes as $operacao)
                            <tr>
                            <th scope="row" title="Editar operações"><a href={{ URL::route($rotaAlterar, array('id' => $operacao->id )) }}>{{$operacao->id}}</a></th>
                              <td>{{$operacao->nome}}</td>
                              <td>{{$operacao->ativo == 'A' ? 'Ativo' : 'Inativo'}}</td>
...
@else
    ...
    <form id="{{ $tela }}" action="{{ $tela == 'alterar' ? $rotaAlterar : $rotaIncluir }}" method="post">
        @csrf
        @if($tela == 'alterar')
            <input type="hidden" name="id" value="{{ $operacoes[0]->id ?? '' }}">
        @endif
        <div class="container">
            <div class="row row-cols-md-3 g-3">
                <div class="col-md-4">
                    <label for="pessoa" class="form-label">Nome*</label>
                    <input type="text" class="form-control" id="nome" name="nome" maxlength="200" required value="{{ $operacoes[0]->nome ?? '' }}">
                </div>
            </div>
            ...
            <div class="row mt-1">
                <div class="col-md-2">
                    ...
                    <select class="form-control" id="ativo" name="ativo">
                        <option value="0" {{ isset($operacoes[0]->ativo) && $operacoes[0]->ativo == '0' ? 'selected' : '' }}>Inativo</option>
                        <option value="1" {{ isset($operacoes[0]->ativo) && $operacoes[0]->ativo == '1' ? 'selected' : '' }}>Ativo</option>
                    </select>
                </div>
            </div>
