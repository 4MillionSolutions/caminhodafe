                ...
                <table class="table table-striped text-center">
                  ...
                  @if(isset($tecnicos))
                        @foreach ($tecnicos as $tecnico)
                            <tr>
                            <th scope="row" title="Editar tecnico"><a href={{ URL::route($rotaAlterar, array('id' => $tecnico->id )) }}>{{$tecnico->id}}</a></th>
                              <td>{{$tecnico->nome}}</td>
                              <td>{{$tecnico->ativo == 'A' ? 'Ativo' : 'Inativo'}}</td>
...
@else
    ...
    <form id="{{ $tela }}" action="{{ $tela == 'alterar' ? $rotaAlterar : $rotaIncluir }}" method="post">
        @csrf
        @if($tela == 'alterar')
            <input type="hidden" name="id" value="{{ $tecnicos[0]->id ?? '' }}">
        @endif
        <div class="container">
            <div class="row row-cols-md-3 g-3">
                <div class="col-md-4">
                    <label for="pessoa" class="form-label">Nome*</label>
                    <input type="text" class="form-control" id="nome" name="nome" maxlength="200" required value="{{ $tecnicos[0]->nome ?? '' }}">
                </div>
            </div>
            ...
            <div class="row mt-1">
                <div class="col-md-2">
                    ...
                    <select class="form-control" id="ativo" name="ativo">
                        <option value="0" {{ isset($tecnicos[0]->ativo) && $tecnicos[0]->ativo == '0' ? 'selected' : '' }}>Inativo</option>
                        <option value="1" {{ isset($tecnicos[0]->ativo) && $tecnicos[0]->ativo == '1' ? 'selected' : '' }}>Ativo</option>
                    </select>
                </div>
            </div>
